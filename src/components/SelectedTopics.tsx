...
const SelectedTopics = (props: Props) => {
  const handleClick = (selectedTopic: string) => {
    if(props.mode === 'creation') {
      props.setSelectedTopics(props.selectedTopics.filter((topic) => topic !== selectedTopic));
    }
  }
  return (
    <div className="flex my-2 flex-wrap">
      {props.selectedTopics.map((selectedTopic, index) => (
        ...
        >
          ...
          {
            props.mode === 'creation' && <MdOutlineCancel onClick={() => handleClick(selectedTopic)} className="cursor-pointer h-4 w-4 text-slate-300 dark:text-gray-800" />
          }
        </div>
      ))}
    </div>
