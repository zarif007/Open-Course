...
const CourseDetailsCreation = () => {
  ...
  const submitData = async () => {
    const course = {
      ...
      topics: courseTopics.filter((courseTopic) => courseTopic.id !== 0),
    };

    const result = await axios.post(`${v1MainEndpoint}/course`, course)
  };
  return (
    <div className=" border-slate-300 dark:border-gray-800 rounded my-4 p-3 md:p-6">
      ...
      <div className="flex justify-center md:justify-end">
        <Button variant="general" className="px-12 py-6 w-full md:w-fit mx-0" onClick={submitData}>
