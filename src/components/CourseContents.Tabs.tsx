...
import { setCurrentCourseTopicForView, setEnrollState } from '@/redux/features/course-view-slice';
...
const CourseContentsTabs = () => {
  ...
  const handleNextButton = async () => {
    ...
    try {
      const { data } = await axios.get(`${v1MainEndpoint}/enrollState?user=${user?.id}&course=${course.id}`);
      ...
      const state: IEnrollState = {
        ...
        finishedTopics: enrollState.finishedTopics.includes(currentCourseTopicId.toString()) ? enrollState.finishedTopics : 
          [...enrollState.finishedTopics, currentCourseTopicId.toString()],
      }

      const updatedEnrollState = await axios.put(`${v1MainEndpoint}/enrollState`, state);

      dispatch(setEnrollState(updatedEnrollState.data.data));

      router.push(`/course/${course.slug}?topicId=${currentCourseTopicId + 1}`)

      dispatch(setCurrentCourseTopicForView(course.topics[currentCourseTopicId]))
    } catch (error) {
      console.log(error)
    } finally {
      setIsLoading(false);
  ...
  return (
    <Tabs defaultValue="content" className="w-full mx-auto px-2 md:px-6">
      <LargeHeading size="lg">{course.title}</LargeHeading>
      <TabsList className="">
        ...
        <div className="mt-20 flex justify-end">
          {
            currentCourseTopic.topicID && currentCourseTopic.topicID < course.topics.length ? (
              <Button className="px-12" onClick={handleNextButton} isLoading={isLoading}>Next</Button>
            ) : (
              <Button className="px-12" onClick={handleDoneButton} isLoading={isLoading}>Done</Button>
            )
          }
        </div>
      </TabsContent>
